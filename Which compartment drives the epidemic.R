...
repeat{
  ...
  d = rbind(d,data.frame(InfectionsA, InfectionsP, InfectionsS)) #Input each individual trial's outcome into data frame
  
  
  if(Student_ID == 100){break}
}
...
boxplot<- ggplot(meltedtable,aes(x=variable,y=value))+geom_boxplot()
